...
begin
   if (not ChrArr[2].Selected) and (ChrArr[2].Valid) then begin
      OldIndex := 2;   
      FrmMain.SelectChr(ChrArr[2].UserChr.Name);//2004/05/17
      ChrArr[0].Selected := FALSE;
      ChrArr[1].Selected := FALSE;
...
begin
   ...
   ChrArr[2].FreezeState := TRUE; //±âº»ÀÌ ¾ó¾î ÀÖ´Â »óÅÂ
   ChrArr[0].Selected := TRUE;
   ChrArr[1].Selected := FALSE;
   ChrArr[2].Selected := FALSE;
         ...
         case ChrArr[n].UserChr.Job of
            0: begin
               if ChrArr[n].UserChr.Sex = 0 then begin
                  ...
                  by := (SCREENHEIGHT - 600) div 2 + 211{75-23}; //³²ÀÚ
                  fx := bx;
                  fy := by;
               end else begin
                  bx := (SCREENWIDTH - 800) div 2 + 121{65};
                  by := (SCREENHEIGHT - 600) div 2 + 197{75-2-18};  //¿©ÀÚ  µ¹»óÅÂ
                  fx := bx;
                  fy := by;    //¿òÁ÷ÀÌ´Â »óÅÂ
               end;
            end;
