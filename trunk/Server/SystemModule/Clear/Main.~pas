...
begin
  ...
  if Application.MessageBox('是否确认要清除注册信息，清除后需要重新注册才能正常使用？？？',
    ...
    if DeleteFile(sWindowsDirectory + 'UserLicense.ini') and ClearRegistry then Application.MessageBox('清除成功！！！', '提示信息', MB_ICONQUESTION) else Application.MessageBox('清除失败！！！', '提示信息', MB_ICONQUESTION);
  end;
end;
