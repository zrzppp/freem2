    ...
    for I := m_MerchantList.Count - 1 downto 0 do begin
      ...
      if Merchant.m_PEnvir <> nil then begin
        ...
        if Merchant.m_boAddtoMapSuccess and (not Merchant.m_boIsHide) then begin
          MainOutMessage('商人初始化失败...' + Merchant.m_sCharName + ' ' + Merchant.m_sMapName + '(' + IntToStr(Merchant.m_nCurrX) + ':' + IntToStr(Merchant.m_nCurrY) + ')');
        ...
        MainOutMessage(Merchant.m_sCharName + ' - 商人初始化失败... (m.PEnvir=nil)');
        m_MerchantList.Delete(i);
        Merchant.Free;
      end;
      FrmMain.Caption:=sCaption + '[正在初始化NPC(' + IntToStr(m_MerchantList.Count-I) + '/' + IntToStr(m_MerchantList.Count) + ')]';
      // Do Not UnComment!
...
begin
  for I := QuestNPCList.Count - 1 downto 0 do begin
    ...
    if NormNpc.m_PEnvir <> nil then begin
      ...
      MainOutMessage(NormNpc.m_sCharName + ' Npc初始化失败... (npc.PEnvir=nil) ');
      QuestNPCList.Delete(i);
      NormNpc.Free;
    end;
...
begin
  try
    if (GetTickCount() - dwProcessUsrLogTick) > 8*60*1000 {8 mins} then begin
      ...
      WriteLn(LogFile, ';名字, 性别, Class, 攻击模式, 等级, 地图, Co-Ord, 用户ID, IP, GM');
