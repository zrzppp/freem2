...
begin
  ...
  if not IsIPaddr(Gateaddr) then begin
    Application.MessageBox('网关地址设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditGateAddr.SetFocus;
    exit;
  end;

  if (GatePort < 0) or (GatePort > 65535) then begin
    Application.MessageBox('网关端口设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditGatePort.SetFocus;
    exit;
  end;

  if not IsIPaddr(IDSAddr) then begin
    Application.MessageBox('管理服务器地址设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditIDSAddr.SetFocus;
    exit;
  end;

  if (IDSPort < 0) or (IDSPort > 65535) then begin
    Application.MessageBox('管理服务器端口设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditIDSPort.SetFocus;
    exit;
  end;

  if not IsIPaddr(DBAddr) then begin
    Application.MessageBox('数据库服务器地址设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditDBAddr.SetFocus;
    exit;
  end;

  if (DBPort < 0) or (DBPort > 65535) then begin
    Application.MessageBox('数据库服务器端口设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditDBPort.SetFocus;
    exit;
  end;

  if not IsIPaddr(LogServerAddr) then begin
    Application.MessageBox('日志服务器地址设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditLogServerAddr.SetFocus;
    exit;
  end;

  if (LogServerPort < 0) or (LogServerPort > 65535) then begin
    Application.MessageBox('日志服务器端口设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditLogServerPort.SetFocus;
    exit;
  end;

  if not IsIPaddr(MsgSrvAddr) then begin
    Application.MessageBox('游戏主服务器地址设置错误！！！','Error!',MB_OK + MB_ICONERROR);
    EditMsgSrvAddr.SetFocus;
    exit;
  end;

  if (MsgSrvPort < 0) or (MsgSrvPort > 65535) then begin
    Application.MessageBox('游戏主服务器端口设置错误！！！','Error!',MB_OK + MB_ICONERROR);
