...
begin
  if (IDSocket.Address <> '') and not (IDSocket.Active) then IDSocket.Active := True;
end;

procedure TFrmIDSoc.IDSocketRead(Sender: TObject; Socket: TCustomWinSocket);//004A183C
begin
...
begin
  ...
  if IDSocket.Socket.Connected then IDSocket.Socket.SendText(sSendText);
end;
//004A1718
  ...
  for i := 0 to GlobaSessionList.Count - 1 do begin
    ...
    if GlobaSessionInfo <> nil then begin
      ...
      if (GlobaSessionInfo.sAccount = sAccount) and
        (GlobaSessionInfo.nSessionID = nSessionID) then begin
        Result := True;
        break;
      end;
    end;
...
begin
  if IDSocket.Socket.Connected then begin
    IDSocket.Socket.SendText('(' + IntToStr(SS_SERVERINFO) + '/' +
      sServerName + '/' + '99' + '/' + IntToStr(FrmUserSoc.GetUserCount) + ')');
  end;
...
begin
  ...
  for i := 0 to GlobaSessionList.Count - 1 do begin
    ...
    if GlobaSessionInfo <> nil then begin
      if (GlobaSessionInfo.sAccount = sAccount) and
        (GlobaSessionInfo.sIPaddr = sIPaddr) then begin
        Result := True;
        break;
      end;
    end;
