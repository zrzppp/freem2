...
begin
  ...
  if MessageDlg(' «∑Ò»∑»œ…æ≥˝»ÀŒÔ ' + sChrName + ' £ø', mtConfirmation,
...
begin
  ...
  if (n8 >= 1) and (ChrGrid.RowCount - 1 >= n8) then s10 := ChrGrid.Cells[1, n8];
...
begin
  sChrName := EdChrName.Text;
  if sChrName = '' then exit;
  if MessageDlg(' «∑Ò»∑»œΩ˚”√»ÀŒÔ ' + sChrName + ' £ø', mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
...
begin
  sChrName := EdChrName.Text;
  if sChrName = '' then exit;
  if MessageDlg(' «∑Ò»∑»œ∆Ù”√»ÀŒÔ ' + sChrName + ' £ø', mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
        nIndex := HumChrDB.Index(sChrName);
...
begin
  ...
  try
    if HumChrDB.Open then begin
      if HumChrDB.ChrCountOfAccount(FrmCreateChr.sUserId) < g_nMaxCreateChar then begin
...
begin
  sChrName := EdChrName.Text;
  if sChrName = '' then exit;
  if MessageDlg(' «∑Ò»∑»œ…æ≥˝»ÀŒÔ ' + sChrName + ' º∞»ÀŒÔ ˝æ›£ø',
...
begin
  ...
  if MessageDlg(' «∑Ò»∑»œΩ˚”√º«¬º ' + IntToStr(nIndex) + ' £ø',
    mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
...
begin
  ...
  if HumDBRecord.boDeleted then ChrGrid.Cells[4, nRowCount] :=
      DateTimeToStr(HumDBRecord.dModDate)
  else
...
procedure TFrmIDHum.BtnChrIDSearchClick(Sender: TObject);
var
  sAccount:    string;
  ChrList:     TStringList;
  i, nIndex:   integer;
