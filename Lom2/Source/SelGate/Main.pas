...
begin
  ...
  if IsConnLimited(sRemoteIPaddr) then begin
    ...
    MainOutMessage('Connected: ' + sRemoteIPaddr,1);
    exit;
  end;
...
begin
  ...
  if Application.MessageBox('你是否退出服务?',
                            'Confirm',
    ...
    while (True) do begin
     ...
     for nSocketIndex:= 0 to GATEMAXSESSION - 1 do begin
       ...
       if (GetTickCount - g_SessionArray[nSocketIndex].dwConnctCheckTick) > dwKeepConnectTimeOut then begin
         ...
         MainOutMessage('Timed out/Blocked: ' + sRemoteIPaddr,1);
         Continue;
       end;
...
begin
  if Application.MessageBox('你要停止服务吗?',
                            'Confirm',
...
begin
  if Application.MessageBox('你要清除日志吗?',
                            'Confirm',
