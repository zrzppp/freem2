...
begin
  ...
  end else if Sender = ListViewRoute then begin
    ProcessListViewSelect();
  end else if Sender = ButtonAddRoute then begin
    ProcessAddRoute();
  end else if Sender = ButtonEdit then begin
    ProcessEditRoute();
  end else if Sender = ButtonOK then begin
    Close();
  end;
end;
...
begin
  ...
  if nNulIdx >= 20 then begin
    MessageBox(Handle, 'Cannot add more routes, maximum has been reached.',
