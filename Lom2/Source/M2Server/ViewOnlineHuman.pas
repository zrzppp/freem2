...
begin
  ...
  for I := 0 to ViewList.Count - 1 do begin
    ...
    GridHuman.Cells[11,I + 1]:=IntToStr(PlayObject.m_nBankGold);
...
begin
  ...
  if sHumanName = '' then begin
    Application.MessageBox('Please enter a valid name!','Error!',MB_OK + MB_ICONEXCLAMATION);
    exit;
  end;
  for I := 0 to ViewList.Count - 1 do begin
    PlayObject:=TPlayObject(ViewList.Objects[I]);
...
begin
  ...
  if (nSelIndex < 0) or (ViewList.Count <= nSelIndex) then begin
    Application.MessageBox('Please select a user from the list first!','Selection required',MB_OK + MB_ICONINFORMATION);
    exit;
  end;
  sPlayObjectName:=GridHuman.Cells[1,nSelIndex + 1];
  PlayObject:=UserEngine.GetPlayObject(sPlayObjectName);
  if PlayObject = nil then begin
    Application.MessageBox('User is no longer online','User offline',MB_OK + MB_ICONINFORMATION);
    exit;
  end;
